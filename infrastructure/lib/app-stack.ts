...
export class PfaAppStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: AppStackProps) {
    ...
    userData.addCommands(
      ...
      "",
      "# ── Step 1: Install ALL packages needed by deployment scripts ────────────",
      "# This MUST happen BEFORE the CodeDeploy agent starts, because the agent",
      "# immediately tries to run after-install.sh which calls python3.12 and pip.",
      "# Running all installs in one dnf call is faster than multiple calls.",
      "dnf install -y python3.12 python3.12-pip python3.12-devel gcc git ruby wget jq amazon-cloudwatch-agent",
      "",
      "# ── Step 2: Application user & directories ───────────────────────────────",
      "id -u pfa &>/dev/null || useradd -r -s /sbin/nologin -d /opt/pfa pfa",
      "mkdir -p /opt/pfa /etc/pfa",
      "chown pfa:pfa /opt/pfa",
      "chmod 750 /etc/pfa",
      "",
      "# ── Step 3: CodeDeploy agent (starts after all deps are installed) ───────",
      // Use IMDSv2 token to fetch region (required when token-only mode is active)
      ...
      "",
      "# ── Step 4: System update (slow — runs last so it doesn't block deployments) ──",
      "dnf update -y",
      "",
    ...
    const asg = new autoscaling.AutoScalingGroup(this, "AppAsg", {
      ...
      healthChecks: autoscaling.HealthChecks.withAdditionalChecks({
        ...
        // 600s grace period — gives user data enough time to install packages
        // and start the CodeDeploy agent before ELB health checks begin.
        gracePeriod: cdk.Duration.seconds(600),
      }),
