...
export class PfaAppStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: AppStackProps) {
    ...
    userData.addCommands(
      ...
      "",
      "# ── CodeDeploy agent (installed FIRST so it is ready before dnf update finishes) ──",
      // Use IMDSv2 token to fetch region (required when LaunchTemplate defaults to IMDSv2)
      ...
      "",
      "# ── System update & remaining packages ───────────────────",
      "dnf update -y",
      "dnf install -y python3.12 python3.12-pip python3.12-devel",
      "dnf install -y gcc git jq",
      "",
      "# ── CloudWatch agent ─────────────────────────────────────",
